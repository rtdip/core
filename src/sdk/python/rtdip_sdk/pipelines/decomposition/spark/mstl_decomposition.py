...
class MSTLDecomposition(DecompositionBaseInterface):
    ...
    def _resolve_single_period(self, period_spec: Union[int, str], group_pdf: pd.DataFrame) -> int:
        ...
        period_spec : Union[int, str]
            Period specification (integer or string name)
        group_pdf : pd.DataFrame
            Pandas DataFrame for the group
        ...
        if isinstance(period_spec, str):
            return self._resolve_string_period(period_spec, group_pdf)
    ...
    def _resolve_string_period(self, period_spec: str, group_pdf: pd.DataFrame) -> int:
    ...
    def _resolve_periods(self, group_pdf: pd.DataFrame) -> List[int]:
        ...
        resolved_periods = [
            self._resolve_single_period(period_spec, group_pdf)
            for period_spec in periods_input
        ]

        self._validate_periods(resolved_periods, group_pdf)
