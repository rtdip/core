...
def evaluate_autogluon(predictor, test_df, freq='h'):
    ...
    print(f"Note: These are forecasts beyond the test set, not aligned with test timestamps")
...
def compute_residuals(predictions_df, test_df, model_name):
    ...
    if len(merged) == 0:
        print(f"   No exact timestamp matches for {model_name}, using hourly normalization")
        predictions_df['timestamp_normalized'] = predictions_df['timestamp'].dt.floor('h')
    ...
    return merged[['item_id', 'timestamp', 'target', 'predicted', 'residual', 'model_name']]
...
def generate_comparison_report(ag_metrics, lstm_metrics, xgboost_metrics, ag_leaderboard, output_dir):
...
def main():
    ...
    try:
        ...
        xgboost_training_duration = (datetime.now() - xgboost_start_time).total_seconds()

        # 6. Evaluate AutoGluon
        ag_metrics, ag_leaderboard, ag_predictions_df = evaluate_autogluon(ag_predictor, test_df, freq=FREQ)
        ...
        # 8. Evaluate XGBoost
        xgboost_metrics, xgboost_predictions_df = evaluate_xgboost(xgboost_model, test_df)
        ...
        print(f"  Note: AutoGluon predictions are future forecasts, not aligned with test timestamps")
        ...
        print(f"  Note: LSTM predictions are future forecasts, not aligned with test timestamps")
        ...
        print(f"  Note: XGBoost predictions are future forecasts, not aligned with test timestamps")
