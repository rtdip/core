...
def test_methods(mocker: MockerFixture):
    ...
    #     }
    # date_strings = ['2023-10-01T12:00:00', '2023-10-02T12:00:00', '2023-10-03T12:00:00', '2023-10-04T12:00:00']
    ...
    # date_index = pd.to_datetime(date_strings)
    # df = pd.DataFrame(data, index=date_index)
    # x = db.write_realised(df, "Location_A_System_1") # influx

    # data = {'output': [1.21, 1.21, 1.21, 1.21],
    #         'system': ['Location_A_System_1', 'Location_A_System_1', 'Location_A_System_1', 'Location_A_System_1']}
    # date_strings = ['2023-10-01T12:00:00', '2023-10-02T12:00:00', '2023-10-03T12:00:00', '2023-10-04T12:00:00']
    # date_index = pd.to_datetime(date_strings)
    # df = pd.DataFrame(data, index=date_index)
    # x = db.write_realised_pvdata(df=df, region='') # influx

    # x = db.write_kpi() # influx

    # data = {
    # 'forecast': [1.21, 1.22, 1.23, 1.21],
    # 'pid': [313, 313, 317, 317],
    # 'customer': ['Location_A', 'Location_A', 'Location_A', 'Location_A'],
    # 'type': ['demand', 'demand', 'demand', 'demand']
    # }
    # date_strings = ['2023-10-10T12:00:00', '2023-10-10T18:00:00', '2023-10-11T12:00:00', '2023-10-11T18:00:00']
    # date_index = pd.to_datetime(date_strings)
    # df = pd.DataFrame(data, index=date_index)
    # df = pd.read_csv('/Users/Rodalyn.Barce/Documents/RTDIP/openstef/prediction.csv', index_col=0)
    # df.index = pd.to_datetime(df.index)
    # x = db.write_forecast(data=df, t_ahead_series=True) # influx

    # client = EntsoePandasClient(api_key="9cbc6eb1-6b03-413e-b1c4-ef05ee3c8f11")
    # start = pd.Timestamp('20230101', tz='Europe/Brussels')
    # end = pd.Timestamp('20231001', tz='Europe/Brussels')
    # country_code = 'NL'
    ...
    df["year_created"] = df.index.strftime("%Y-%m-%d")

    def rename(x):
        if x.startswith("P0.01_"):
            return x[len("P0.01_") :]
        return x
    ...
    # x = db.get_prediction_jobs() # error
    # x = db.get_prediction_job(317) # error
    ...
    #     predictor_groups=["load_profiles"],  # market_data, weather_data, load_profiles
    # )
    ...
    #####
    # splitting methods
    # x = db.get_wind_ref('Deelen',
    #                     datetime(2023, 8, 29),
    #                     datetime(2023, 8, 30))
    # x = db.get_energy_split_coefs({"id": "317"})
    # x = db.get_input_energy_splitting(pj=pj,
    #                                   datetime_start=datetime(2024, 8, 29),
    #                                   datetime_end=datetime(2024, 8, 30)) # influx
    ...
    #     start_time=datetime(2023, 8, 29),
    #     end_time=datetime(2023, 8, 30),
    #     pj={"id": "317", "quantiles": [2, 3], "resolution_minutes": 3}
    # )
    ...
    #     datetime_start=datetime(2023, 9, 4),) # [PARSE_SYNTAX_ERROR] Syntax error at or near ''distance''.(line 2, pos 240)

    #####
    # systems methods
    # x = db.get_systems_near_location((51.813,5.837)) # [PARSE_SYNTAX_ERROR] Syntax error at or near ''distance''.(line 2, pos 240)
