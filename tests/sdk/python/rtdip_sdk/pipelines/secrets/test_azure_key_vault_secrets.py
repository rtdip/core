...
def test_azure_key_vault_secret_setup(mocker: MockerFixture):
    ...
    assert azure_key_vault.libraries() == Libraries(pypi_libraries=[PyPiLibrary(
...
def test_azure_key_vault_secrets_sert(mocker: MockerFixture):
    ...
    hashicorp_vault = AzureKeyVaultSecrets(vault="akv-vault-url", credential=object(), key="vault-key", secret="vault-secret")
