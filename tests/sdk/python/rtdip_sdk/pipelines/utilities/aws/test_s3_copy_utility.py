...
def test_s3_copy_utility():
    ...
    rnd_source_domain_name: str = ".".join(
        "".join(random.choice(letters_and_numbers) for _ in range(9)) for _ in range(3)
    )  # NOSONAR
    rnd_destination_domain_name: str = ".".join(
        "".join(random.choice(letters_and_numbers) for _ in range(9)) for _ in range(3)
    )  # NOSONAR
    rnd_keys: str = "".join(
        "".join(random.choice(letters_and_numbers) for _ in range(4)) for _ in range(3)
    )  # NOSONAR
    rnd_object_name: str = (
    ...
    conn.create_bucket(Bucket=rnd_source_domain_name, CreateBucketConfiguration={
        'LocationConstraint': 'us-east-1'})
    conn.create_bucket(Bucket=rnd_destination_domain_name, CreateBucketConfiguration={
        'LocationConstraint': 'us-east-1'})
