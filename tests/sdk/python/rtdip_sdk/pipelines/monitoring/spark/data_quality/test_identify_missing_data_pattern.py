...
from src.sdk.python.rtdip_sdk.pipelines.monitoring.spark.data_quality.identify_missing_data_pattern import IdentifyMissingDataPattern
...
def test_invalid_patterns(spark, caplog):
    ...
    with pytest.raises(ValueError) as exc_info, caplog.at_level(logging.ERROR, logger='IdentifyMissingDataPattern'):
        monitor.check()

    assert "Each pattern must have a 'second' key for 'minutely' frequency." in str(exc_info.value)
