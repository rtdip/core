...
from src.sdk.python.rtdip_sdk.pipelines.monitoring.spark.data_quality.identify_missing_data_interval import IdentifyMissingDataInterval
...
def test_missing_intervals_with_given_interval(spark, caplog):
    ...
    actual_logs = [record.message for record in caplog.records if record.levelname == "INFO" and record.name == "IdentifyMissingDataInterval"]

    assert len(expected_logs) == len(actual_logs), f"Expected {len(expected_logs)} logs, got {len(actual_logs)} "
