...
def test_spark_pi_omf_write_setup():
    delta_merge_destination = SparkPIOMFDestination(
        None, {}, test_url, "username", "password", 1, create_type_message=False
    )
...
def test_spark_pi_omf_pre_batch_records_for_api_call(spark_session: SparkSession):
    expected_df = spark_session.createDataFrame(
        schema=expected_schema, data=expected_data
    )
    df = SparkPIOMFDestination(
        expected_df, {}, test_url, "username", "password", 1, create_type_message=False
    )._pre_batch_records_for_api_call(expected_df)
    assert df.collect()[0]["payload"] == {}
...
def test_spark_rest_api_write_batch_fails(
    ...
    mocker.patch(
        "src.sdk.python.rtdip_sdk.pipelines.destinations.spark.PI_OMF.SparkPIOMFDestination._api_micro_batch",
