...
def test_spark_pcdm_to_delta_write_setup(spark_session: SparkSession):
    pcdm_to_delta_destination = SparkPCDMToDeltaDestination(spark_session, None, {}, destination_float= "test_delta_merge_destination_setup_float", destination_string= "test_delta_merge_destination_setup_string", mode = "append")
...
def test_spark_pcdm_to_delta_write_batch_append(spark_session: SparkSession):
    ...
    pcdm_to_delta_destination = SparkPCDMToDeltaDestination(spark_session, test_df, {}, destination_float="test_spark_pcdm_to_delta_write_batch_append_float",destination_string= "test_spark_pcdm_to_delta_write_batch_append_string", mode="overwrite", merge=False)
    ...
    actual_string_df = spark_session.table("test_spark_pcdm_to_delta_write_batch_append_string")
...
def test_spark_pcdm_to_delta_write_batch_merge(spark_session: SparkSession):
    ...
    delta_destination = SparkDeltaDestination(create_data_float_df, {}, mode="overwrite", destination="test_spark_pcdm_to_delta_write_batch_merge_float")
    ...
    delta_destination = SparkDeltaDestination(create_data_string_df, {}, mode="overwrite", destination="test_spark_pcdm_to_delta_write_batch_merge_string")
    delta_destination.write_batch()
    ...
    test_df = spark_session.createDataFrame([
        ...
        {"EventDate": datetime(2023, 1, 20).date(), "TagName": "Tag3", "EventTime": datetime(2023, 1, 20, 2, 0), "Status": "Good", "Value": "1.05", "ValueType": "float", "ChangeType": "delete"}, 
        {"EventDate": datetime(2023, 1, 20).date(), "TagName": "Tag2", "EventTime": datetime(2023, 1, 20, 1, 0), "Status": "Good", "Value": "test", "ValueType": "string", "ChangeType": "insert"},
        {"EventDate": datetime(2023, 1, 20).date(), "TagName": "Tag4", "EventTime": datetime(2023, 1, 20, 4, 0), "Status": "Good", "Value": "test", "ValueType": "string", "ChangeType": "delete"}
    ])
    ...
    expected_string_df = spark_session.createDataFrame([{"EventDate": datetime(2023, 1, 20).date(), "TagName": "Tag2", "EventTime": datetime(2023, 1, 20, 1, 0), "Status": "Good", "Value": "test"}]).select("EventDate", "TagName", "EventTime", "Status", "Value")
    pcdm_to_delta_destination = SparkPCDMToDeltaDestination(spark_session, test_df, {}, destination_float= "test_spark_pcdm_to_delta_write_batch_merge_float", destination_string= "test_spark_pcdm_to_delta_write_batch_merge_string", mode="update", merge=True)
    pcdm_to_delta_destination.write_batch()
    actual_float_df = spark_session.table("test_spark_pcdm_to_delta_write_batch_merge_float")
    assert expected_float_df.schema == actual_float_df.schema
    assert expected_float_df.collect() == actual_float_df.collect()

    actual_string_df = spark_session.table("test_spark_pcdm_to_delta_write_batch_merge_string")
    assert expected_string_df.schema == actual_string_df.schema
    assert expected_string_df.collect() == actual_string_df.collect()

def test_spark_pcdm_to_delta_write_stream_append(spark_session: SparkSession, mocker: MockerFixture):
    mocker.patch("src.sdk.python.rtdip_sdk.pipelines.destinations.spark.pcdm_to_delta.SparkDeltaDestination", return_value=MockDeltaDestination())
    expected_df = spark_session.createDataFrame([
        {"EventDate": datetime(2023, 1, 20).date(), "TagName": "Tag1", "EventTime": datetime(2023, 1, 20, 1, 0), "Status": "Good", "Value": "1.02", "ValueType": "float", "ChangeType": "insert"},
    ])
    pcdm_to_delta_destination = SparkPCDMToDeltaDestination(spark_session, expected_df, {}, destination_float= "test_spark_pcdm_to_delta_write_batch_merge_float", destination_string= "test_spark_pcdm_to_delta_write_batch_merge_string", mode="append", merge=False)
    actual = pcdm_to_delta_destination.write_stream()
    assert actual is None

def test_spark_pcdm_to_delta_write_stream_merge(spark_session: SparkSession, mocker: MockerFixture):
    ...
    pcdm_to_delta_destination = SparkPCDMToDeltaDestination(spark_session, expected_df, {}, destination_float= "test_spark_pcdm_to_delta_write_batch_merge_float", destination_string= "test_spark_pcdm_to_delta_write_batch_merge_string", mode="update", merge=True)
    actual = pcdm_to_delta_destination.write_stream()
    assert actual is None
