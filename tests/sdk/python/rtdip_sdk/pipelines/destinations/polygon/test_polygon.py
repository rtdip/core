...
def test_polygon_write_batch(mocker: MockerFixture):
    ...
    mocker.patch('src.sdk.python.rtdip_sdk.pipelines.destinations.polygon', return_value=None)
    # mocker.patch('EVMContractDestination.write_batch', new_callable=mocker.Mock(return_value=mocker.Mock(account=mocker.Mock(return_value=mocker.Mock(sign_transaction=mocker.Mock(return_value=None))))))
    # mocker.patch('web3.eth', new_callable=mocker.Mock(return_value=mocker.Mock(send_raw_transaction=mocker.Mock(return_value=None))))
    # mocker.patch('web3.eth', new_callable=mocker.Mock(return_value=mocker.Mock(wait_for_transaction_receipt=mocker.Mock(return_value=None))))
