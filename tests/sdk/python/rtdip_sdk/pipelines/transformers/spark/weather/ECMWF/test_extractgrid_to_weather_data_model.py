...
import numpy as np
import xarray as xr
from src.sdk.python.rtdip_sdk.pipelines.transformers.spark.weather.ECMWF.extractgrid_to_weather_data_model import ECMWFExtractGridToWeatherDataModel
...
def test_constructor():
    ...
    assert (extract.lat == xr.DataArray(np.linspace(lat_min, lat_max, int(np.round((lat_max - lat_min) / grid_step)) + 1), dims=["latitude"])).all()
    assert (extract.lon == xr.DataArray(np.linspace(lon_min, lon_max, int(np.round((lon_max - lon_min) / grid_step)) + 1), dims=["longitude"])).all()
