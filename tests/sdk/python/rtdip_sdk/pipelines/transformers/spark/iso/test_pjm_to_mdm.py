...
def test_pjm_to_mdm_usage(spark_session: SparkSession):
    ...
    input_df: DataFrame = spark_session.read.csv(f"{base_path}/input.csv", header=True, schema=PJM_SCHEMA)
...
def test_pjm_to_mdm_meta_with_params(spark_session: SparkSession):
    ...
    transformer: BaseRawToMDMTransformer = PJMToMDMTransformer(spark_session,
                                                               input_df,
                                                               output_type="meta",
                                                               version=version,
                                                               series_id=series_id,
