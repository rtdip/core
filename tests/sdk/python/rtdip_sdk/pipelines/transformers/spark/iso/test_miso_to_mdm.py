...
def test_miso_to_mdm_usage(spark_session: SparkSession):
    ...
    expected_df: DataFrame = spark_session.read.csv(f"{base_path}/output.csv", header=True, schema=MDM_USAGE_SCHEMA)
...
def test_miso_to_mdm_invalid_output_type(spark_session: SparkSession):
    ...
    transformer: BaseRawToMDMTransformer = MISOToMDMTransformer(spark_session, data=input_df, output_type="raw")

    with pytest.raises(ValueError) as exc_info:
        transformer.transform()

    assert str(exc_info.value) == "Invalid output_type `raw` given. Must be one of ['usage', 'meta']"
