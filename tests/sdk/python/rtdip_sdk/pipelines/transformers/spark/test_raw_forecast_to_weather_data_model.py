...
def test_raw_forecast_to_weather_data_model(spark_session: SparkSession):
    expected_df: DataFrame = spark_session.read.csv(f"{parent_base_path}/output.csv", header=True,
                                                    schema=WEATHER_DATA_MODEL)
    input_df: DataFrame = spark_session.read.csv(f"{parent_base_path}/input.csv", header=True,
