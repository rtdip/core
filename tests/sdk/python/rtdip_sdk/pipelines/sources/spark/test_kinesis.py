...
from src.sdk.python.rtdip_sdk.pipelines._pipeline_utils.constants import KINESIS_SCHEMA
...
def test_spark_kinesis_read_setup(spark_session: SparkSession):
    ...
    kinesis_source = SparkKinesisSource(kinesis_configuration)
...
def test_spark_kinesis_read_stream_fails(spark_session: SparkSession, mocker: MockerFixture):
    kinesis_configuration = kinesis_configuration_dict
    kinesis_source = SparkKinesisSource(spark_session, kinesis_configuration)
    mocker.patch.object(kinesis_source, "spark", new_callable=mocker.PropertyMock(return_value=mocker.Mock(readStream=mocker.Mock(format=mocker.Mock(return_value=mocker.Mock(options=mocker.Mock(return_value=mocker.Mock(load=mocker.Mock(side_effect=Exception)))))))))
    with pytest.raises(Exception):
