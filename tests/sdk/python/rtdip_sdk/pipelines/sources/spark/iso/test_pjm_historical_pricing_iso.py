...
def test_pjm_historical_pricing_iso_read_setup(spark_session: SparkSession):
    iso_source = PJMHistoricalPricingISOSource(spark_session, iso_configuration)
...
def test_pjm_historical_pricing_iso_read_real_time(
...
def test_pjm_historical_pricing_iso_read_day_ahead(
    spark_session: SparkSession, mocker: MockerFixture
):
    iso_source = PJMHistoricalPricingISOSource(
...
def test_pjm_historical_pricing_iso_iso_fetch_url_fails(
    spark_session: SparkSession, mocker: MockerFixture
):
    base_iso_source = PJMHistoricalPricingISOSource(spark_session, iso_configuration)
...
def test_miso_historical_pricing_iso_invalid_dates(spark_session: SparkSession):
    with pytest.raises(ValueError) as exc_info:
        iso_source = PJMHistoricalPricingISOSource(
            spark_session, {**iso_configuration, "start_date": "20230101"}
        )
    ...
    with pytest.raises(ValueError) as exc_info:
        iso_source = PJMHistoricalPricingISOSource(
            spark_session,
            {**iso_configuration, "start_date": "2023-03-01", "end_date": future_date},
        )
        iso_source.pre_read_validation()
