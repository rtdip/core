...
import sys
sys.path.insert(0, '.')
from src.sdk.python.rtdip_sdk.pipelines.sources.python.delta_sharing import PythonDeltaSharingSource
...
def test_python_delta_sharing_setup():
    delta_sharing_source = PythonDeltaSharingSource(profile_path, share_name, schema_name, table_name)
    assert delta_sharing_source.system_type().value == 1
    assert delta_sharing_source.libraries() == Libraries(maven_libraries=[], pypi_libraries=[], pythonwheel_libraries=[])
...
def test_python_delta_sharing_read_batch(mocker: MockerFixture):
    delta_sharing_source = PythonDeltaSharingSource(profile_path, share_name, schema_name, table_name)
    mocker.patch.object(delta_sharing, 'load_as_pandas', return_value = pd.DataFrame({'test': ['test_data']}))
