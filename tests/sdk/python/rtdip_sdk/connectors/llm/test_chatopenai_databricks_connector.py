...
def test_connection_close(mocker: MockerFixture):
    ...
    mocker.patch(LANGCHAIN_SQLDATABASE, return_value = MockSQLDatabase(engine=MockEngine()))
...
def test_cursor_close(mocker: MockerFixture):
    ...
    cursor = ChatOpenAIDatabricksSQLCursor(ChatOpenAIDatabricksConnection(catalog=CATALOG, schema=SCHEMA, server_hostname=SERVER_HOSTNAME, http_path=HTTP_PATH, access_token=ACCESS_TOKEN, openai_api_key=OPENAI_API_KEY).connection)
    cursor.close()
...
def test_cursor_execute_fails(mocker: MockerFixture):
    ...
    mocked_cursor = ChatOpenAIDatabricksSQLCursor(ChatOpenAIDatabricksConnection(catalog=CATALOG, schema=SCHEMA, server_hostname=SERVER_HOSTNAME, http_path=HTTP_PATH, access_token=ACCESS_TOKEN, openai_api_key=OPENAI_API_KEY).connection)
