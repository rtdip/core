...
MOCKED_QUERY='SELECT TagName, EventTime, Interpolated_Value as Value FROM (SELECT *, lag(EventTime) OVER (PARTITION BY TagName ORDER BY EventTime) AS Previous_EventTime, lag(Value) OVER (PARTITION BY TagName ORDER BY EventTime) AS Previous_Value, lead(EventTime) OVER (PARTITION BY TagName ORDER BY EventTime) AS Next_EventTime, lead(Value) OVER (PARTITION BY TagName ORDER BY EventTime) AS Next_Value, CASE WHEN Next_EventTime IS NULL THEN Previous_Value WHEN Previous_EventTime IS NULL and Next_EventTime IS NULL THEN NULL ELSE (Next_Value - Previous_Value) * ((unix_timestamp(EventTime) - unix_timestamp(Previous_EventTime)) / (unix_timestamp(Next_EventTime) - unix_timestamp(Previous_EventTime))) END AS Interpolated_Value FROM (SELECT coalesce(a.TagName, b.TagName) as TagName, coalesce(a.EventTime, b.EventTime) as EventTime, b.Status, b.Value FROM (SELECT explode(array(from_utc_timestamp(to_timestamp(\'2011-01-01T00:00:00+00:00\'), "+0000"), from_utc_timestamp(to_timestamp(\'2011-01-02T23:59:59+00:00\'), "+0000"))) AS EventTime, explode(array("MOCKED-TAGNAME")) AS TagName) a FULL OUTER JOIN (SELECT * FROM (SELECT * FROM ssip.time_series WHERE EventDate BETWEEN date_sub(to_date(to_timestamp(\'2011-01-01T00:00:00+00:00\')), 1) AND date_add(to_date(to_timestamp(\'2011-01-02T23:59:59+00:00\')), 1) AND TagName in (\'MOCKED-TAGNAME\'))) b ON a.EventTime = b.EventTime AND a.TagName = b.TagName))WHERE EventTime in (from_utc_timestamp(to_timestamp(\'2011-01-01T00:00:00+00:00\'), "+0000"), from_utc_timestamp(to_timestamp(\'2011-01-02T23:59:59+00:00\'), "+0000"))'
