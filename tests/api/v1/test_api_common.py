...
from src.api.v1.common import lookup_before_get, query_mapping_endpoint, split_table_name, concatenate_dfs_and_order
...
def test_api_lookup_before_get(mocker):
    ...
    mock_get_data = [
        ...
        pd.DataFrame(
            {
                "EventTime": [RAW_MOCKED_PARAMETER_DICT["start_date"], RAW_MOCKED_PARAMETER_DICT["start_date"]],
    ...
    # Set-up mocker
    mocker = mocker_setup(mocker, MOCK_METHOD, mock_get_data, patch_side_effect=mock_get_data, tag_mapping_data=MOCK_TAG_MAPPING_MULTIPLE) 
    mocker.patch(MOCK_BATCH_METHOD, return_value=mock_get_data)
    
    # Get result from lookup_before_get function
    connection = DatabricksSQLConnection(access_token="token", server_hostname="test", http_path="test")
    actual = lookup_before_get("raw", connection, test_parameters)

    # Define expected result
    expected = pd.DataFrame(
        {
            "EventTime": [RAW_MOCKED_PARAMETER_DICT["start_date"], RAW_MOCKED_PARAMETER_DICT["start_date"], RAW_MOCKED_PARAMETER_DICT["end_date"]],
...
def test_api_common_query_mapping_endpoint(mocker):
    ...
    # Set-up mocker
    mocker_setup(mocker, MOCK_METHOD, test_data={}, tag_mapping_data=MOCK_TAG_MAPPING_MULTIPLE) 

    # Run the function
    tags = ["Tagname1", "Tagname2"]
    connection = DatabricksSQLConnection(access_token="token", server_hostname="test", http_path="test")
    actual = query_mapping_endpoint(tags, MOCK_MAPPING_ENDPOINT_URL, connection=connection)
...
def test_api_common_split_table_name():
    ...
    actual_with_expected_format = split_table_name("test.sensors.asset_restricted_events_float")
    ...
    assert actual_with_incorrect_format_missing.value.args[0] == expected_with_incorrect_format_message
    assert actual_with_incorrect_schema.value.args[0] == expected_with_incorrect_format_message
...
def test_api_common_concatenate_dfs_and_order_unpivoted():
    ...
    actual = concatenate_dfs_and_order(dfs_arr=[test_df1, test_df2], tags=["TestTag1", "TestTag2", "TestTag3"], pivot=False)
